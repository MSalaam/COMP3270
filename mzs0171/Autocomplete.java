...
public class Autocomplete {
   ...
   public static class BinarySearchAutocomplete implements Autocompletor {
     ...
     */
     ...
     // @Sources of studying:
     // https://docs.google.com/presentation/d/18zxXEeOXWGQVUteA-R0D5iAlN1Fg8kLHlDIh3GDNC9g/edit#slide=id.g442eb61d9d_0_0
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
            ...
            // For invalid/null inputs
         if (word == null){
            throw new NullPointerException("Input is invalid");
         }
            
            // For invalid weight inputs
         if (weight < 0){
         ...
         for (int i = 0; i < word.length(); i++) {
            ...
            // place nodes in next node
            if (nextNode == null) { 
               nextNode = new Node(index, currentNode, weight);
               currentNode.children.put(index, nextNode);
            }
            
            // To set new Max weight
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         if (k == 0) {
            ...
            // Following node q to last available character. Empty arrai if none
         ...
         if (currentNode.isWord){
            terms.add(new Term(currentNode.myWord, currentNode.myWeight));
         }
            
            // Populating queue with currentNode children
         for (Node x: currentNode.children.values()) {
            if (x != null) {
               nodeQ.offer(x);
            }
         }
         ...
         while (nodeQ.size() > 0) {
            ...
            // Comparing weight and reordeing in descending order
      ...
      public String topMatch(String prefix) {
         ...
         for (char character: prefix.toCharArray()) {
            if (currentNode.children.containsKey(character) == false) {
               return "";
            }
         ...
         while (currentNode.isWord == false & currentNode.getWeight() != currentNode.mySubtreeMaxWeight) {
            for (Node x: currentNode.children.values()) {
               if (x.mySubtreeMaxWeight == currentNode.mySubtreeMaxWeight) {
                  currentNode = x;
                  break;
               }
            }
      ...
      public double weightOf(String term) {
         ...
         // Following characters to last node in trie
